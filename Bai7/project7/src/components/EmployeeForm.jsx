...
function EmployeeForm({ onSubmit, initialData, onCancel }) {
  const { departments, positions } = useEmployeeStore()
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-4 animate-slide-up" style={{ animationDelay: '300ms' }}>
        <button
          type="button"
          onClick={onCancel}
          className="px-6 py-2.5 rounded-lg border border-teal-200 text-teal-600 hover:bg-teal-50 transition-all duration-200"
        >
          Hủy
        </button>
        <button
          type="submit"
          className="px-6 py-2.5 bg-teal-500 text-white rounded-lg hover:bg-teal-600 transition-all duration-200 hover:scale-[1.02] shadow-lg shadow-teal-200"
        >
